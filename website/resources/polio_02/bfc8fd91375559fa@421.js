...
function _svg(d3,width,height,margin,x,y,groupedData,line2,color,events_data,counts_array)
{
  ...
  g.append("rect")
    .style("fill", "#4682b4") // rect's fill color
    .attr("height", 20) // rect's height (in pixels)
    .attr("width", 20) // rect's width (in pixels)
    .attr("x", x(events_data[6]["Year"]) + 20) // x position of the top-left corner
    .attr("y", y(d3.max(counts_array))); // y position of the top-left corner

  g.append("text")
    .attr("y", y(d3.max(counts_array)) + 15) //magic number here
    .attr("x", x(events_data[6]["Year"]) + 50)
    .attr("text-anchor", "right")
    .attr("font-size", 14)
    .attr("font-weight", "bold")
    .text("Cases");
  ...
  g.append("rect")
    .style("fill", "#fc8d62") // rect's fill color
    .attr("height", 20) // rect's height (in pixels)
    .attr("width", 20) // rect's width (in pixels)
    .attr("x", x(events_data[6]["Year"]) + 20) // x position of the top-left corner
    .attr("y", y(d3.max(counts_array)) + 40); // y position of the top-left corner

  g.append("text")
    .attr("y", y(d3.max(counts_array)) + 55) //magic number here
    .attr("x", x(events_data[6]["Year"]) + 50)
    .attr("text-anchor", "right")
    .attr("font-size", 14)
    .attr("font-weight", "bold")
